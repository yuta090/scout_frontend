...
const CustomerList: React.FC<CustomerListProps> = ({ customers, onEdit, onDelete, onAdd, onBulkUpload }) => {
  ...
  const [checkProgress, setCheckProgress] = useState({ current: 0, total: 0, message: '' });
  ...
  const handleAuthCheck = async (customer: Customer, platform: 'airwork' | 'engage') => {
    ...
    try {
      ...
      // Update the local state to reflect the new auth status
      const updatedStatus = result.success ? 'authenticated' : 'failed';
      ...
      if (customerIndex !== -1) {
        customers[customerIndex] = {
          ...customers[customerIndex],
          [platform === 'airwork' ? 'airwork_auth_status' : 'engage_auth_status']: updatedStatus
        };
      }
      
      // Show success/error message to user
      if (!result.success) {
        // You can add a toast notification here if you want to show the error message
  ...
  return (
    <div className="bg-white shadow rounded-lg">
      ...
      {checkProgress.total > 0 && (
        <div className="px-6 py-3 bg-indigo-50 border-t border-b border-indigo-100">
          ...
          {filteredCustomers.map((customer) => (
            <li key={customer.id} className="px-4 py-4 sm:px-6 hover:bg-gray-50 transition-colors duration-150">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                  {/* Authentication Status and Check Buttons */}
