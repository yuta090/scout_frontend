...
const CustomerForm: React.FC<CustomerFormProps> = ({ customer, onSubmit, onCancel, isOpen }) => {
  ...
  useEffect(() => {
    if (customer) {
      ...
      // 新規作成時は一時的なIDを生成
      setTempCustomerId(crypto.randomUUID());
    }
  }, [customer]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Ensure airwork_login is properly structured
      const submissionData = {
        ...formData,
        airwork_login: {
          username: formData.airwork_login?.username?.trim() || '',
          password: formData.airwork_login?.password?.trim() || ''
        }
      };
  ...
  const handleAirworkLoginChange = (airworkLogin: { username?: string; password?: string }) => {
    setFormData({
      ...formData,
      airwork_login: {
        username: airworkLogin.username || '',
        password: airworkLogin.password || ''
      }
    });
  };
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <AuthenticationSection 
              ...
              customerId={tempCustomerId || undefined}
              isLoading={isLoading}
              authError={authError}
            />
          </div>
