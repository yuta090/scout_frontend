...
ADD COLUMN airwork_auth_status text NOT NULL DEFAULT 'pending' CHECK (airwork_auth_status IN ('pending', 'authenticated', 'failed')),
ADD COLUMN engage_auth_status text NOT NULL DEFAULT 'pending' CHECK (engage_auth_status IN ('pending', 'authenticated', 'failed'));
